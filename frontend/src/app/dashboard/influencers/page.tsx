...
export default function InfluencersPage() {
  ...
  // Removed WhatsApp service functions
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="flex items-center gap-2 bg-muted border border-border text-foreground px-4 py-2 rounded-lg font-medium hover:bg-muted/80 transition-all">
            <Download size={18} />
            <span>Export CSV</span>
          </button>
          <button 
            onClick={() => { resetForm(); setEditingInfluencer(null); setShowModal(true); }}
            className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg font-medium hover:opacity-95 transition-all shadow-sm">
            <Plus size={18} />
        ...
        <div className="flex gap-4 mb-6">
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-2 bg-muted border border-border px-4 py-2 rounded-lg hover:bg-muted/80 transition-all">
              <Filter size={18} />
              <span>{statusFilter === 'ALL' ? 'All Status' : statusFilter}</span>
            </button>
            {showFilterDropdown && (
              <div className="absolute right-0 mt-2 w-40 bg-card border border-border rounded-xl shadow-2xl z-30 overflow-hidden">
                {['ALL', 'PENDING', 'APPROVED', 'REJECTED'].map(s => (
                  <button key={s} onClick={() => { setStatusFilter(s); setShowFilterDropdown(false); }}
                    className={`w-full text-left px-4 py-2.5 text-sm transition-all hover:bg-muted ${statusFilter === s ? 'text-primary bg-primary/10 font-semibold' : 'text-muted-foreground'}`}>
                    {s === 'ALL' ? 'All Status' : s}
                  </button>
                ))}
              </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                const plat = getPlatform(inf);
                return (
                  <tr key={inf.id} className="border-b border-border hover:bg-muted/40 transition-colors group">
                    ...
                    <td className="py-4 text-muted-foreground text-sm">
                      <div className="flex flex-col gap-1">
                        ...
                        {inf.whatsapp && <span className="flex items-center gap-1 text-muted-foreground">Phone: {inf.whatsapp}</span>}
                      </div>
                    </td>
                    <td className="py-4 text-right">
                      <div className="flex items-center justify-end gap-2 relative">
                        <button onClick={() => setOpenMenuId(openMenuId === inf.id ? null : inf.id)} className="p-2 hover:bg-muted rounded-lg text-muted-foreground transition-colors"><MoreVertical size={16} /></button>
                        {openMenuId === inf.id && (
                          <div className="absolute right-0 top-full mt-1 w-40 bg-card border border-border rounded-xl shadow-xl z-20 overflow-hidden text-left">
                            {inf.status === 'PENDING' && (
                              <>
                                <button onClick={() => handleStatusChange(inf.id, 'APPROVED')} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-emerald-500 hover:bg-emerald-50 transition-all">
                                  <CheckCircle size={14} /> Approve
                                </button>
                                <button onClick={() => handleStatusChange(inf.id, 'REJECTED')} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-500 hover:bg-red-50 transition-all border-b border-border/50">
                                  <XCircle size={14} /> Reject
                                </button>
                              </>
                            )}
                            <button onClick={() => openEditModal(inf)} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-muted-foreground hover:bg-muted transition-all mt-1">
                              <Pencil size={14} /> Edit Details
                            </button>
                            <button onClick={() => handleDelete(inf.id)} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-500 hover:bg-red-50 transition-all">
      ...
      <AnimatePresence>
        {showModal && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto"
            ...
            <motion.div className="relative z-10 w-full max-w-2xl bg-card border border-border rounded-2xl p-8 max-h-[90vh] overflow-y-auto my-8 shadow-2xl text-foreground"
              initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}>
              <div className="flex items-center justify-between mb-6 sticky top-0 bg-card/90 backdrop-blur-md pt-2 pb-4 -mt-4 border-b border-border z-20">
                <h2 className="text-2xl font-bold">{editingInfluencer ? 'Edit Influencer' : 'Add Influencer'}</h2>
                <button onClick={() => { setShowModal(false); setEditingInfluencer(null); }} className="p-2 hover:bg-muted rounded-lg"><X size={20} /></button>
              </div>
              <form className="space-y-6" onSubmit={editingInfluencer ? handleUpdate : handleCreate}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input value={newInf.name} onChange={e => setNewInf(p => ({ ...p, name: e.target.value }))}
                        ...
                        className="w-full bg-background border border-border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-primary transition-all" />
                    </div>
                    <div>
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-secondary border-b border-border pb-2">Contact Details</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-primary border-b border-border pb-2">Platform & Audience</h3>
                  ...
                  <div>
                    ...
                    <textarea value={newInf.audienceStats} onChange={e => setNewInf(p => ({ ...p, audienceStats: e.target.value }))}
                      ...
                      className="w-full bg-background border border-border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-primary transition-all resize-none" />
                  </div>
                </div>

                {editingInfluencer && (
                  <div className="space-y-4 border-t border-border pt-4">
                    <h3 className="text-lg font-semibold text-emerald-600 border-b border-border pb-2">Portal Access Credentials</h3>
                ...
                <div className="pt-4 mt-4 border-t border-border sticky bottom-0 bg-card/90 backdrop-blur-md pb-4 z-20">
                  <button type="submit" disabled={isSubmitting} className="w-full py-3 bg-primary text-white rounded-xl font-bold hover:opacity-95 transition-opacity shadow-sm disabled:opacity-70 flex items-center justify-center gap-2">
                    {isSubmitting ? (
                      <>
